...
const LAYOUT_OPTIONS: {
  ...
  { value: "bar", label: "Bar", description: "Single row" },
  { value: "grid", label: "Grid", description: "Multi-column" },
  { value: "marquee", label: "Marquee", description: "Scrolling" },
];

type StyleUpdate = <K extends keyof PresswallConfig>(
  key: K,
...
export function HeadingStyleSection({
  config,
  onUpdate,
}: {
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-3">
        <div>
          <p className="text-sm">Show label</p>
          <p className="text-muted-foreground text-xs">Caption above logos</p>
        </div>
        <Switch
...
export function LayoutStyleSection({
  ...
  return (
    <>
      ...
      <ControlRow hint={`${config.gap}px`} label="Spacing">
        <Slider
          max={64}
          min={8}
...
export function LogosStyleSection({
  config,
  onUpdate,
}: {
  config: PresswallConfig;
  ...
  return (
    <>
      ...
      {config.colorMode === "muted" ? (
        <ControlRow hint={`${config.grayscaleOpacity}%`} label="Muted opacity">
          <Slider
            max={100}
            min={20}
            onValueChange={(value) =>
...
export function ContainerStyleSection({
  config,
  onUpdate,
}: {
  config: PresswallConfig;
        ...
        <ControlRow label="Text color">
          <div className="flex gap-2">
            <input
              ...
              type="color"
              value={
                config.textColor.startsWith("#") ? config.textColor : "#111111"
              }
            />
            <Input
